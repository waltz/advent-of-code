...
func main() {
	...
	for scanner.Scan() {
		...
		fmt.Println(report)
		isSafe := determineSafety(report)

		// fmt.Println("initial safety?", isSafe)

		if isSafe == false {
			// fmt.Println("not safe, checking variants")
			for i, _ := range report {
				variant := []int{}
				firstHalf := report[:i]
				secondHalf := report[i+1:]
				variant = append(variant, firstHalf...)
				variant = append(variant, secondHalf...)

				isVariantSafe := determineSafety(variant)

				if isVariantSafe {
					// fmt.Println("found a variant that is safe", variant)
					isSafe = true
					break
				}
			}
		...
		fmt.Println("final safety", isSafe)
